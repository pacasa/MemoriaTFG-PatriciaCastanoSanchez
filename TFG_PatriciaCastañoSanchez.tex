...
En la figura~\ref{fig:image02} se ve la información que proviene de la base de datos Oracle, es la información ‘válida’ que debe contener el directorio activo. Se requiere especial atención en las columnas I y Q ya que, a propósito, se han modificado para mostrar el funcionamiento del script.
...
En la figura~\ref{fig:image04} se muestra el script en curso que va sacando trazas durante su ejecución indicando a qué colección se está añadiendo cada equipo y un contador para ver cuántos equipos está añadiendo.
...
En la figura~\ref{fig:image05} se muestra los equipos que ha añadido el script a la Colección 1 (beta ITUser grupo 1) en la herramienta de SCCM.
\\
...
En la figura~\ref{fig:image06} se muestra la fecha en la que caduca la contraseña para el usuario utilizado en este ejemplo.
...
En la figura~\ref{fig:image07} se puede ver el correo electrónico que envía el script al usuario al que le va a caducar la contraseña.
\\
...
En la figura~\ref{fig:image11} aparece la Central Administration donde aparecen todos los certificados generados y se realza el certificado que acaba de generar el script.
\\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=17cm, keepaspectratio]{img/ListaSPvacia.png}
	\caption{Lista Sharepoint destino}
	\label{fig:ListaSPvacia}
\end{figure}

En la figura~\ref{fig:ListaSPvacia} se muestra la lista destino, vacía antes de ejecutar el script.
...
En la figura~\ref{fig:image13} se muestra la lista origen, de la que se copiarán los ítems
...
\begin{figure}
	\centering
	\includegraphics[width=17cm, keepaspectratio]{img/ListaSPllena.png}
	\caption{Lista Sharepoint destino tras ejecutar el script}
	\label{fig:ListaSPllena}
\end{figure}

En la figura~\ref{fig:ListaSPllena} vemos cómo se ha rellenado la lista que inicialmente estaba vacía.
\\
